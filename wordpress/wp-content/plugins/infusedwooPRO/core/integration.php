	...
	class IA_Woocommerce extends WC_Integration {	
		...
		function recent_update_routines() {
			if(is_admin()) {   
				if($this->enabled == "yes") {
					...
					if($perform_routines) {
						$this->ia_app_connect();
						do_action("infusedwoo_update_routines", INFUSEDWOO_PRO_VER );
					}
				}
			}
		}
		...
		function validate_settings_fields( $form_fields = false ) { 
			 ...
			 foreach ( $form_fields as $k => $v ) {
				...
				if ( method_exists( $this, 'validate_' . $v['type'] . '_field' ) ) {
					if(isset($_POST["woocommerce_infusionsoft_$k"])) {
						$field = $this->{'validate_' . $v['type'] . '_field'}( $k, $_POST["woocommerce_infusionsoft_$k"] );
					} else {
						$field = '';
					}
					$this->sanitized_fields[$k] = $field;
				 } else {
					 $this->sanitized_fields[$k] = $_POST["woocommerce_infusionsoft_$k"];
				 }
			 }
		 }
		...
		public function admin_options() {
		 ...
		 public function process_admin_options() {
	        ...
	        if(isset($this->sanitized_fields['infusedwoo_save'])) {
