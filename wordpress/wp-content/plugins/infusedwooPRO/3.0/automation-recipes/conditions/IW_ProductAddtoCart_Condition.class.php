...
class IW_ProductAddtoCart_Condition extends IW_Automation_Condition {
	function get_title() {
		return 'When product added to cart is ...';
	}
	...
	function on_class_load() {
		add_action( 'wp_ajax_infusedwoo_data_src_wooproducts', 'iw_data_search_wooproduct',10 );
	}
	...
	function test($config, $trigger) {
		...
		$quantity = $trigger->pass_vars[2];
		$variation_id = $trigger->pass_vars[3];
...
iw_add_condition_class('IW_ProductAddtoCart_Condition');