...
class IW_OrderPreviousStatus_Condition extends IW_Automation_Condition {
	function get_title() {
		return 'If order\'s previous status is ...';
	}
	...
	function test($config, $trigger) {
		return ("wc-". $trigger->pass_vars[1]) == $config['status'];		
	}
}

iw_add_condition_class('IW_OrderPreviousStatus_Condition');