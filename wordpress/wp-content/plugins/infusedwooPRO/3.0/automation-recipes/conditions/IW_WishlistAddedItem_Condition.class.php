...
class IW_WishlistAddedItem_Condition extends IW_Automation_Condition {
	function get_title() {
		return 'Item is added to user\'s wishlist basket ...';
	}
	...
	function validate_entry($conditions) {
		...
		} else if($conditions['type'] == 'wptype') {
	...
	function test($config, $trigger) {
		if($trigger->pass_vars[0] != 'add_item') return false;

		$product_id = $trigger->pass_vars[2];			
...
iw_add_condition_class('IW_WishlistAddedItem_Condition');