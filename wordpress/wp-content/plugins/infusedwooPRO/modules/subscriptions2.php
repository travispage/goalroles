...
function ia_woocommerce_sub_regular_price($price, $product) {
	...
	if($ifstype == 'Subscription') {
		...
		$sign_up_fee = (int) get_post_meta($product_id, 'infusionsoft_sign_up_fee', 	true);
...
function ia_woocommerce_sub_filter( $price, $product ){		
	...
	if($ifstype == 'Subscription') {
		...
		$ncycles = isset($nsub['NumberOfCycles']) ? $nsub['NumberOfCycles']*$freq : 0;
		...
		} else return  $price . " / {$freq}{$stringCycle}{$ncycle_str}"; 
		...
		foreach($woocommerce->cart->cart_contents as $item) {
			...
			if($ifstype == 'Subscription') {	
				...
				switch($sub['DefaultCycle']) {						
						case 1: 
							$stringCycle = 'year'; 	
							$nextbill = (strtotime("+1 year", time())-time())/(24*3600); 
							break;						
						case 2: 
							$stringCycle = 'month'; 
							$nextbill = (strtotime("+1 month", time())-time())/(24*3600); 
							break;						
				...
				if($incl_disc != 'no') $sub_price = $woocommerce->cart->get_discounted_price( $item, $sub_price, $add_totals = false );
