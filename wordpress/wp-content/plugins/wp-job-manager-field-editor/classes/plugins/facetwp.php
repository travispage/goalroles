			...
			case 'fpdate':
				...
				if( isset( $field_data[ 'picker_mode'] ) && $field_data['picker_mode'] === 'range' ){
					...
					if( strpos( $val, $separator ) === false ){
						...
						$possible_separators = apply_filters( 'job_manager_field_editor_fpdate_range_separator', array( ' to ', ' bis ', ' kuni ', ' έως ', ' до ', ' até ', ' do ', ' ถึง ' ), $meta_key, $field_data, $params, $class );
					...
					if( isset( $date_array ) && ! empty( $date_array[0] ) && ! empty( $date_array[1] ) ){
						...
						// Get epoch using the saved format (or format in settings)
						$from_date = WP_Job_Manager_Field_Editor_Fields_Date::get_epoch( $date_array[0], $meta_key );
						$to_date = WP_Job_Manager_Field_Editor_Fields_Date::get_epoch( $date_array[1], $meta_key );
						// FacetWP dates must be in YYYY-MM-DD format!
						$params['facet_value'] = date( 'Y-m-d', $from_date );
						$params['facet_display_value'] = date( 'Y-m-d', $to_date );
				 ...
				 * Multiple Dates
				 */
				if ( isset( $field_data['picker_mode'] ) && $field_data['picker_mode'] === 'multiple' ){
					...
					if( strpos( $val, ';' ) !== FALSE ){
						...
						$this->index_loop( $val_array, $params, $class );
						return false;
					}
