<tr id="<?php echo esc_attr( sanitize_title( $this->plugin_slug . '_licence_key_row' ) ); ?>" class="active plugin-update-tr smylesv2-updater-licence-key-tr">
	<td class="plugin-update" colspan="3">
		...
		<div class="smylesv2-updater-licence-key">
			<label for="<?php echo sanitize_title( $this->plugin_slug ); ?>_licence_key"><?php _e( 'License', 'wp-job-manager-emails' ); ?>:</label>
			...
			<span class="description"><?php _e( 'Enter your license key and email and hit return. A valid key is required for updates.', 'wp-job-manager-emails' ); ?> <?php printf( 'Lost your key? <a href="%s">Retrieve it here</a>.', esc_url( 'https://plugins.smyl.es/lost-api-key/' ) ); ?></span>
		</div>
	</td>
	<script>
		jQuery(function(){
