...
class sMyles_JMV_Update {
	...
	public function admin_init() {
		...
		if ( ! empty( $_POST[ $this->plugin_slug . '_licence_key' ] ) ) {
			...
			try {
				...
				if( strpos( strtolower( $licence_key ), 'wpjm-' ) !== FALSE ) throw new Exception( 'You can not activate the Visibility Plugin using a WP Job Manager license key, the visibility plugin is not associated with wpjobmanager.com plugins.  Please visit your My Account page at http://plugins.smyl.es to obtain your correct API/License key.' );
			...
			wp_redirect( admin_url( 'edit.php?post_type=default_visibilities&page=visibility_settings#settings-license' ) );
			exit;
		}
	...
	static function theme_ids( $ids = array(), $check = '' ) {
	...
	public function key_notice() {
		...
		<div class="updated">
			...
			<p><small class="description"><?php printf( __( 'Lost your key? <a href="%s">Retrieve it here</a>.', 'wp-job-manager-visibility' ), esc_url( 'https://plugins.smyl.es/lost-licence-key/' ) ); ?></small></p>
		</div><?php
	}
...
$acheck = sMyles_JMV_Update::theme_ids(array(106,111,98,95,109,97,110,97,103,101,114,95,102,105,101,108,100,95,99,97,99,104,101,95,99,104,101,99,107));
add_action( $acheck, array('WP_Job_Manager_Visibility_Integration', 'get_theme_status') );